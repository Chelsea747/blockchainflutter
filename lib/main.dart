...
class _LoginPageState extends State<LoginPage> {
  ...
  signInUp({required String endpoint}) {
    var url = Uri.parse('https://Blockchain.nathanl4.repl.co/$endpoint');
    ...
    var body = json.encode({
      'username': username,
      'password': digest.toString(), //for hash: digest.toString()
    });
    ...
    http.post(url, headers: headers, body: body).then((response) {
      if (response.statusCode == 200) {
        ...
        SharedVar.username = username;
        SharedVar.password = digest.toString();
        ...
        showDialog(
            ...
            builder: (context) => AlertDialog(
                  title: Text('Failed'),
                  content: Text('Failed to sign in/up, ${response.body}'),
                ));
      }
    });
  }
